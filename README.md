...
1. 安装前请先彻底删除旧版插件：卸载插件时务必勾选 `「同时删除插件配置文件」` 和 `「同时删除插件持久化数据」`，避免旧配置或旧数据残留。
2. 通过 AstrBot 插件管理使用 `zip` 压缩包或仓库链接安装。
3. AstrBot 通常会自动安装 `requirements.txt` 中的依赖；如果依赖安装失败，请根据日志手动安装。
4. 如果需要使用 `查询题目`，请在服务器中安装 Chromium，并确保插件能够找到 Chromium 可执行文件。
5. 在 AstrBot WebUI 中填写插件配置。
6. 重载或重启 AstrBot。
7. 首次使用前必须先执行（大约需要 1 分钟）：
...
`查询用户` 生成的个人主页卡片会根据用户总 AC 题数切换背景主题，这部分样式魔改自 [ACMBot](https://github.com/suzmii/ACMBot)：
...
1. 安装前请先彻底删除旧版插件：卸载插件时务必勾选 `「同时删除插件配置文件」` 和 `「同时删除插件持久化数据」`，避免旧配置或旧数据残留。
2. 使用前请先正确配置 MySQL 和 NYOJ 接口账号信息，并执行 `nyoj初始化` 初始化数据。
3.  `查询题目` 需要服务器中有可用的 Chromium。
